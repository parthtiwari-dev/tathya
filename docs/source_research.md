...
| PM India News Updates | Official | `https://www.pmindia.gov.in/en/news-updates/` | Registered - disabled, adapter added | Official PMO-managed website. Adapter has fixture tests for server-rendered listing/article pages; live activation still needs local audit because the first transparent request returned HTTP 403. |
| Income Tax Department press releases | Official | `https://www.incometaxindia.gov.in/press-release-rss-feed/-/asset_publisher/bxhj/rss` | Registered - disabled, audit next | Official Ministry of Finance department feed documented on its tax feeds page; test text length and timestamps before activation. |
| RBI press releases | Official regulator | `https://www.rbi.org.in/pressreleases_rss.xml` | Registered - disabled, audit next | RBI documents RSS support for press releases; useful as primary official/regulatory signal, but not a Union Government ministry source. |
